...
当前项目跟进 `@zed-industries/codex-acp` `0.14.x`。该版本跟进 Codex `0.129`，
并会把图片生成作为 tool call 事件发出；项目侧统一按 `tool_call/tool_call_update`
归一成 `tool_progress`，再由微信发送层按角色限频和过滤。
...
| --- | --- | --- | --- |
| `admin-acp` | 开 | 关闭 | 单轮微信可见消息最多 9 条；过程最多 7 条，至少间隔 15 秒；发送 `visible_message_run` 和 `tool_progress` |
| `family-acp` | 关 | 开 | 默认不发 ACP 过程；开 `/output process on` 后最多 4 条，至少间隔 8 秒，只发过滤后的 `visible_message_run`，不发工具进度 |
| `family-api` | 早发默认关 | 开 | 开 `/output family-api-stream on` 后最多提前发 2 条，至少 60 字，至少间隔 2.5 秒，只在强边界切 |

单轮微信发送预算按“最多 9 条可见消息”设计，避免同一个 `context_token`
下撞到微信/iLink 客户端侧大约 10 条后的展示不稳定。预算优先留给最终答案：

- ACP 过程输出按事件流发送，不做 token 级刷新；
- `admin-acp` 过程消息前后更密，中间更少，最多 7 条；
- 最终答案最多保留 2 条，通常是一条完整消息；
- 除 `Context compacted` 这类系统提示会和正文拆开外，最终答案不再按
  `WECHAT_REPLY_CHUNK_CHARS` 做普通长度分段。
...
最终答案默认按完整消息发送，不再按 `WECHAT_REPLY_CHUNK_CHARS` 做普通长度分段。
只有项目识别出的系统提示块会和正文拆成两条，避免系统提示和正式回答挤在同一条
微信消息里。
...
- 过程输出必须限频、限条数、去重，并遵守单轮最多 9 条微信可见消息的预算。
