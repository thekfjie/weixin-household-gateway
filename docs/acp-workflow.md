...
- 目前没有真正的执行中 `/stop`。当前 worker 在处理一条消息时会等待模型调用
  完成；要做即时停止，需要引入 per-session in-flight task 和 API/ACP cancel
  机制。
- `visible_message_run` 是启发式边界，不是协议保证的最终答案标记。
- ACP SDK 是否支持 prompt-level cancel 需要单独验证；如果不支持，只能用断开
  connection 或清 session 的方式近似停止，风险更高。
