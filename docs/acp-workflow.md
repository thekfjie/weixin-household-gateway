...
| --- | --- | --- | --- |
| `admin-acp` | 开 | 关闭 | 最多 20 条过程消息，至少间隔 15 秒；发送 `visible_message_run` 和 `tool_progress` |
...
最终答案仍会按 `WECHAT_REPLY_CHUNK_CHARS` 做兜底分段，默认是 1800 字符。
兜底分段优先在段落、换行、句末或空格处分割；找不到合适切点时才按上限切。
兜底分段目前没有最大条数上限。
...
- ACP prompt 超时后会显式发送 `session/cancel`，并继续等待短时间收集取消前后的
  最后可见文本。若拿到部分内容，会把“已生成内容，可能不完整”发给微信；若 ACP
  没有确认取消，会重置 ACP 连接并清掉该会话的持久 ACP session，避免下一轮继续
  撞上旧任务。
- 目前没有用户可主动触发的执行中 `/stop`。要做即时停止，还需要把 per-session
  in-flight task 暴露给命令层，并同时覆盖 API stream、ACP prompt、typing 状态
  和已排队的过程消息。
...
- 如果以后增加 `/stop`，要同时覆盖 API stream、ACP prompt、typing 状态和已排队
  的过程消息。
